...
handle_ack(struct tcp_stream * a_tcp, struct half_stream * snd,
	   struct half_stream * rcv, u_int acknum)
{
  int ackdiff;
...
process_tcp(u_char * data, int skblen)
{
  ...
  if (iplen - 4 * this_iphdr->ip_hl <sizeof(struct tcphdr)) {
...
{
  int i;
  if (!nids_params.n_tcp_streams) return 0;
  tcp_stream_table_size = nids_params.n_tcp_streams;
...
process_icmp(u_char * data)
{
  ...
  int from_client;
  int len = ntohs(iph->ip_len) - (iph->ip_hl << 2);
